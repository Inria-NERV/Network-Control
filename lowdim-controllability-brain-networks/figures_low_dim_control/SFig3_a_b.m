...
%% This script plots 4 views of the strength + 4 views of the lowdim control centrality and 4 views of the stat diff
...
%%%%% get strength tensor
ll = load('results_data/bigTensor_results_fullBrainControl_on_6134_subjects.mat');
strengthTensor = ll.strengthTensor;
clear ll
...
avgStrength = mean(strengthTensor,2);
...
Z_str = zscore(log((strengthTensor)),0,1); % abs?
Z_low = zscore(log(abs(lowDimTensor)),0,1);


strength_groupAvg = mean((Z_str),2 ,'omitnan' );% abs?
lowControllability_groupAvg = mean((Z_low),2,'omitnan' );

figure;histogram(strength_groupAvg);
hold on;
...
for k =1:n
    ...
    [p,h,stats] = signtest(Z_low(k,:) , Z_str(k,:));

    d = computeCohen_d(Z_low(k,:) , Z_str(k,:), 'paired');
...
for kDim = 1:3
    ...
    if kDim ==1
        controlMetricChosen = ((strength_groupAvg));
        myTitle = 'worst-case';
    elseif kDim ==2
