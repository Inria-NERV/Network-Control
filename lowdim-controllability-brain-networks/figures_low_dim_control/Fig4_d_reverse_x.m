...
allMetricsMean = 0.5*[sizeLateralization;
    mean(worstCaseContLateralization , 'omitnan');mean(lowDimContLateralization , 'omitnan')]';

allMetricsStd = 0.5*[nan(1,nNetworks);...
    std(worstCaseContLateralization , 'omitnan');std(lowDimContLateralization , 'omitnan')]';
...
myBars = barh(allMetricsMean , 'BarWidth' , 0.7 );
...
for kMet = 1:3
    hold on;
    err = errorbar(allMetricsMean(:,kMet) , myBars(kMet).XEndPoints , allMetricsStdLeft(:,kMet), allMetricsStdRight(:,kMet) ,'horizontal', ...
        'k' , 'LineStyle' , 'none');
end
% myBars(1).FaceColor = 
...
ax.YTickLabel = Networks9CellVert;
% ylim([0.5 10])
...
%allMetricsMean(allMetricsMean>0)=0;
...
for kMet = 1:3
    ...
    err = errorbar(allMetricsMean((ii),kMet) , xEnd , barRight(ii), barLeft(ii) ...
        ,'horizontal', ...
        'k' , 'LineStyle' , 'none');
end
% myBars(1).FaceColor = 
